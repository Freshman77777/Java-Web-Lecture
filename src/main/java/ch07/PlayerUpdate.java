 ...
 */
@WebServlet("/jw/ch07/updatePlayer")
public class PlayerUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		data += "            </tr>"
				+ "            <tr>"
				+ "                <td>생년월일</td>";
		data += "<td><input type=\"text\" name=\"bDay\" value=\""+p.getBirDate().toString() +"\" ></td>";
		data += "            </tr>"
				+ "            <tr>"
				+ "                <td>신장</td>";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String birDate_ = request.getParameter("birDate");
		LocalDate birDate = LocalDate.parse(birDate_);
		String height_ = request.getParameter("height");
		int height = Integer.parseInt(height_);
		
		Player p = new Player(backNumber, name, position, birDate, height);
		PlayerDao dao = new PlayerDao();
		dao.updatePlayer(p);
